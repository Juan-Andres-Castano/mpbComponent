	...
	/*prepare local msg */
		...
		vMpbCommandHandlerGetTecStatus_ReturnThruPtr_pucTecIsEnabled(&ucTecIsEnabled);
	 	...
	 	//	TEST_ASSERT_EQUAL_HEX8(mpbAlarmRegister.ucValue[i], Msg.cData[i] );	
			...
			TEST_ASSERT_EQUAL_HEX8(mpbAlarmRegister.ucValue[0], Msg.cData[0] );	
...
void test_mpbCommandHandler_Test_Command_Setup_check_status_Register(void)
{
	//prepare local msg 
	 	Msg.cCommand = GET_STATUS_COMMAND;
